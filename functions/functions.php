...
$con = mysqli_connect("localhost","root","","Ninjoalls") or die("Connection was not established"); 

//function for interesting post
function insertPost(){
    if(isset($_POST['sub'])){
        global $con;
        global $user_id;
        ...
        if(strlen($content)>250){
            echo "<script>alert('Пожалуйста используйте не более 250 слов!')</script>";
            ...
            if(strlen($upload_image) >=1 && strlen($content) >=1){
                ...
                if($run){
                    echo "<script>alert('Ваш пост опубликован!')</script>";
                    echo "<script>window.open('home.php','_self')</script>";
	...
	if(isset($_GET['post_id'])){
		...
		if($p_id != $post_id){
						...
						<div class='panel panel-info'>
							<div class='panel-body'>
								<form action='' method='post' class='form-inline'>
									<textarea placeholder='Напишите ваш комментарий!' class='pb-cmnt-textarea' name='comment'></textarea>
									<button class='btn btn-info pull-right' name='reply'>Комментировать</button>
								</form>
							</div>
						</div>
					</div> 
			...
			if(isset($_POST['reply'])){
				...
				if($comment == ""){
					echo "<script>alert('Напишите комментарий!')</script>";
				...
				<div class='col-sm-6'>
					<div class='row' id='find_people'>
						<div class='col-sm-4'>
							...
							<img class='img-circle' src='users/$user_image' width='150px' height='140px' title='$username' style='flaot: left; margin: 1px;'/>
							</a>
						</div><br><br>
						<div class='col-sm-6'>
